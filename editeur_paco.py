    ...
    def main_loop(self):
        ...
        while not quitter:
            for event in pg.event.get():
                ...
                if pg.mouse.get_pressed(1,0,0) == (1,0,0) and not self.mapOn:
                    # dans l'interface
                ...
                if pg.mouse.get_pressed(0,0,1) == (0,0,1) and not self.mapOn:
                    if self.rectEditeur.collidepoint(pg.mouse.get_pos()):
                        ligne = ((pg.mouse.get_pos()[1]-self.all_levels.levelCourant.camera.etat.y)/HAUTEUR_TILE)
                        colonne = ((pg.mouse.get_pos()[0]-self.all_levels.levelCourant.camera.etat.x)/LARGEUR_TILE)
                        
                        if self.all_levels.levelCourant.matrice[ligne][colonne] != 0 :
                            self.all_levels.levelCourant.matrice[ligne][colonne] = 0
                            self.all_levels.sauvegardeOk = False
